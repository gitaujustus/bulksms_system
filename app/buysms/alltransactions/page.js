...
function AllTransactions() {
  const transactions = [
    { id: 55763, txnRef: 'testfin1', status: 'Success', dr: 0, cr: 100, balBefore: 1206, balAfter: 1306, createdAt: '2024-04-15 08:11' },
    { id: 55764, txnRef: 'testfin2', status: 'Failed', dr: 100, cr: 0, balBefore: 1306, balAfter: 1206, createdAt: '2024-04-16 09:11' },
    { id: 55766, txnRef: 'testfin4', status: 'Success', dr: 20, cr: 200, balBefore: 1356, balAfter: 1556, createdAt: '2024-04-18 11:11' },
    { id: 55767, txnRef: 'testfin5', status: 'Failed', dr: 60, cr: 0, balBefore: 1556, balAfter: 1496, createdAt: '2024-04-19 12:11' },
    { id: 55769, txnRef: 'testfin7', status: 'Success', dr: 30, cr: 70, balBefore: 1536, balAfter: 1576, createdAt: '2024-04-21 14:11' },
    { id: 55770, txnRef: 'testfin8', status: 'Failed', dr: 90, cr: 0, balBefore: 1576, balAfter: 1486, createdAt: '2024-04-22 15:11' },
    { id: 55771, txnRef: 'testfin9', status: 'Success', dr: 0, cr: 200, balBefore: 1486, balAfter: 1686, createdAt: '2024-04-23 16:11' },
    { id: 55772, txnRef: 'testfin10', status: 'Success', dr: 50, cr: 150, balBefore: 1686, balAfter: 1786, createdAt: '2024-04-24 17:11' }
  ];
  ...
  return (
    <main className='bg-[#D9D9D9] min-h-screen p-3 mt-14 md:mt-0 md:ml-80'>
      <div className='bg-white  w-full p-3 border border-black  rounded-md'>
        <h1 className='text-left text-xl md:text-2xl my-5'>Purchases</h1>
        <div className='flex gap-3 font-bold ml-2'>
          ...
          >ALL</p>
          <p 
            className={`pb-2 mx-4 md:px-7 cursor-pointer ${filter === 'Success' ? 'border-b border-red-700' : ''}`}
            onClick={() => setFilter('Success')}
          >SUCCESSFUL</p>
          <p 
            className={`pb-2 mx-4 md:px-7  cursor-pointer ${filter === 'Failed' ? 'border-b border-red-700' : ''}`}
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className="">
                ...
                <th className="px-4 py-2 text-start border border-gray-400 "><p>TXN&nbsp;ref</p></th>
                <th className="px-4 py-2 text-start border border-gray-400">Status</th>
                <th className="px-4 py-2 text-start border border-gray-400">Dr</th>
                <th className="px-4 py-2 text-start border border-gray-400">Cr</th>
                <th className="px-4 py-2 text-start border border-gray-400">Bal&nbsp;before</th>
                <th className="px-4 py-2 text-start border border-gray-400">Bal&nbsp;after</th>
            ...
            <tbody>
              {filteredTransactions.map(transaction => (
                <tr key={transaction.id}>
                  ...
                  <td className="px-4 py-2 border border-black">{transaction.txnRef}</td>
                  <td className="px-4 py-2 border border-black">
                    <p className={`border p-1 rounded-xl ${transaction.status === 'Success' ? 'border-green-600' : 'border-red-600'}`}>
                      {transaction.status}
                    </p>
                  </td>
                  <td className="px-4 py-2 border border-black">{transaction.dr}</td>
                  <td className="px-4 py-2 border border-black">{transaction.cr}</td>
                  <td className="px-4 py-2 border border-black">{transaction.balBefore}</td>
                  <td className="px-4 py-2 border border-black">{transaction.balAfter}</td>
                  <td className="px-4 py-2 border border-black">{transaction.createdAt}</td>
                </tr>
              ))}
            </tbody>
          </table>
