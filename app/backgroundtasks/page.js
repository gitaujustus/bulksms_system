...
function BackgroundTasks() {
  const btasks = [
    { id: 55763,Operationname:'SendCampaign', description: 'You confirmed for 16 Apr 13:03 campaign be sent out', Result: "The long running operation completed successfully", status: 'Completed', RunTime: "0s", createdAt: '2024-04-15 08:11' },
    { id: 55763,Operationname:'Compose SMS', description: 'You composed 16 Apr 13:03 campaign to be sent to 1 destinations', Result: "The long running operation completed successfully", status: 'Completed', RunTime: "0s", createdAt: '2024-04-15 08:11' },
    { id: 55763,Operationname:'SendCampaign', description: 'You confirmed for 16 Apr 13:03 campaign be sent out', Result: "The long running operation completed successfully", status: 'Completed', RunTime: "0s", createdAt: '2024-04-15 08:11' },
    { id: 55763,Operationname:'SendCampaign', description: 'You confirmed for 16 Apr 13:03 campaign be sent out', Result: "The long running operation completed successfully", status: 'Completed', RunTime: "0s", createdAt: '2024-04-15 08:11' },
    { id: 55763,Operationname:'SendCampaign', description: 'You confirmed for 16 Apr 13:03 campaign be sent out', Result: "The long running operation completed successfully", status: 'Completed', RunTime: "0s", createdAt: '2024-04-15 08:11' },
    { id: 55763,Operationname:'SendCampaign', description: 'You confirmed for 16 Apr 13:03 campaign be sent out', Result: "The long running operation completed successfully", status: 'Completed', RunTime: "0s", createdAt: '2024-04-15 08:11' },
  ...
  return (
    <main className='bg-[#D9D9D9] min-h-screen p-3 mt-14 md:mt-0 md:ml-80'>
      <div className='bg-white  w-full p-3 border border-black  rounded-md'>
        <h1 className='text-left text-xl md:text-2xl my-5'>Background Tasks</h1>
          ...
          <table className="w-full table-auto ">
            <thead>
              <tr className="">
                ...
                <th className="px-4 py-2 text-start  border border-gray-400 "><p>Operation&nbsp;name</p></th>
                <th className="px-4 py-2 text-start  border border-gray-400">Description</th>
                <th className="px-4 py-2 text-start  border border-gray-400">Result</th>
                <th className="px-4 py-2 text-start  border border-gray-400">Status</th>
                <th className="px-4 py-2 text-start  border border-gray-400">Run&nbsp;Time</th>
            ...
            <tbody>
              {btasks.map(tasks => (
                <tr key={tasks.id}>
                  <td className="px-4 py-2 border border-black">{tasks.id}</td>
                  <td className="px-4 py-2 border border-black ">{tasks.Operationname}</td>
                  <td className="px-4 py-2 border border-black text-left">{tasks.description}</td>
                  <td className="px-4 py-2 border border-black text-left">{tasks.Result}</td> 
                  <td className="px-4 py-2 border border-black">
                    <p className={`border p-1 rounded-xl text-green-500 border-green-600`}>
                      {tasks.status}
                    </p>
                  </td>
                  <td className="px-4 py-2 border border-black">{tasks.RunTime}</td>
                  <td className="px-4 py-2 border border-black">{tasks.createdAt}</td>
                </tr>
              ))}
            </tbody>
          </table>
